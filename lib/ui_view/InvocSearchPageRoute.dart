...
Future<T?> showInvocSearch<T>({
  required BuildContext context,
  required InvocSearchDelegate<T> delegate,
  String query = '',
}) {
...
/// call. Call [SearchDelegate.close] before re-using the same delegate instance
/// for another [showSearch] call.
abstract class InvocSearchDelegate<T> {
  ...
  _SearchBody? get _currentBody {
    if(_currentBodyNotifier.value != null){
     return _currentBodyNotifier.value!;
    }else{
      return null;
    }
  }
...
class _SearchPageRoute<T> extends PageRoute<T> {
  ...
  final InvocSearchDelegate<T> delegate;
    ...
    return Semantics(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          toolbarTextStyle: theme.primaryTextTheme.displayMedium,
